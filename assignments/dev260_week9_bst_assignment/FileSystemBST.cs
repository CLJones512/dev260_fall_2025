...
namespace FileSystemNavigator
{
    ...
    public class FileSystemBST
    {
        ...
        public bool CreateFile(string fileName, long size = 1024)
        {
            ...
            FileNode fileNode = new FileNode(fileName, FileType.File, size);
            try
            {
        ...
        public bool CreateDirectory(string directoryName)
        {
            ...
            FileNode fileNode = new FileNode(directoryName, FileType.Directory);
            try
            {
                root = InsertNode(root, fileNode);
                return true;
        ...
        public FileNode? FindFile(string fileName)
        {
            ...
            FileNode? fileNode = SearchNode(root, fileName);
            return fileNode;
        }
        ...
        private TreeNode? InsertNode(TreeNode? node, FileNode fileData)
        {
            ...
            {
                node.Right = InsertNode(node.Right, node.FileData);
            }
            return node;
        }
        ...
        private FileNode? SearchNode(TreeNode? node, string fileName)
        {
            ...
            // Recursive case: compare names and go left or right
            int compareStrings = string.Compare(fileName, node!.FileData.Name, StringComparison.OrdinalIgnoreCase);
            if(node == null)
            {
                return null;
            }
            if(fileName == node.FileData.Name)
            {
